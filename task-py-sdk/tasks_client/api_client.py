...
class ApiClient(object):
    ...
    :param retry_backoff_period: (default: 3 sec) Waiting time period between successive retry attempts
    """ # noqa: E501
    ...
    def __init__(self, configuration=None, header_name=None, header_value=None,
                 cookie=None, max_retry_attempts=5, retry_backoff_period=3):
        if configuration is None:
            configuration = Configuration()
        self.configuration = configuration

        self.pool = ThreadPool()
        self.rest_client = rest.RESTClientObject(configuration, max_retry_attempts=max_retry_attempts, retry_backoff_period=retry_backoff_period)
        self.default_headers = {}
        if header_name is not None:
            self.default_headers[header_name] = header_value
        self.cookie = cookie
        self.refresh_cookie = False
        if cookie is None:
            self.refresh_cookie = True

        # Set default User-Agent.
        self.user_agent = 'Swagger-Codegen/16.7.0/python'
    ...
    def update_cookies(self, response_data):
    ...
    def __call_api(
        ...
        if response_data.status == 401:
            ...
            if response_data.status == 401:
                raise tasks_client.rest.ApiException(http_resp=response_data)

        if self.refresh_cookie:
            self.update_cookies(response_data)
    ...
    def __getattr(self, name):
        if name in self.DUPLICATE_SCHEMA_MAPPING:
            return getattr(tasks_client.Ntnx, self.DUPLICATE_SCHEMA_MAPPING[name])
        if hasattr(tasks_client.Ntnx, name.split('.')[-1]):
            return getattr(tasks_client.Ntnx, name.split('.')[-1])
        else:
            return getattr(tasks_client.Ntnx, name)
